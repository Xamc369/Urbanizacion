...
namespace Urbanizacion.Web.Data
{
    public class SeedDb
    {
        ...
        public async Task SeedAsync()
        {
            ...
            await CheckRoles();
            var manager = await CheckUserAsync("10", "JOHAN", "ALTAMIRANO", "tec_inf_johan@hotmail.com", "GENIAL", "0998759275", "1723629240", "Admin");
            var customer = await CheckUserAsync("11", "XIMENA", "MORENO", "gohanalexanderdj@hotmail.com", "GENIAL", "0998759275", "1723629240", "Customer");
        ...
        private async Task CheckNoticiaAsync()
        {
            if (!_dataContext.MarcasAutostbls.Any())
            {
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_Titulo = "Primera" });
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_Autor = "XADI" });
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_Descripcion = "Creacion de pagina web para urbanizacion" });
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_Fecha = DateTime.Now });
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_FechaCreacion = DateTime.Now });
                _dataContext.Noticiastbls.Add(new Noticiastbl { Not_Estado = 'A' });
                await _dataContext.SaveChangesAsync();
